        ...
        col.on('collect', async (i) => {
            ...
            if (value === "=") {
                try {
                    ...
                    data = "";
                    extra = "Something went wrong please try again later or contact the bot developer or press the button clear to restart the calculator.";
                }
            } else if (value === "clear") {
                data = "";
                extra = "Results will be shown here"
            ...
            i.update({ embeds: [new EmbedBuilder().setColor('Purple').setDescription(`\`\`\`\n${data || extra}\n\`\`\``)], components: [row, row1, row2, row3, row4], ephemeral: false })
        })
    }
}